...
import Risu from '../assets/Risu.webp'
...
function Talent() {
    ...
    const items = [
        { src: Talent1, name: "Ariya Risu" }, { src: Talent2, name: "Violetta Lupin" }, { src: Talent3, name: "Maximillian Luxor" }
    ];
    const responsive = {
        superLargeDesktop: {
            ...
            <div className='w-full h-auto relative'>
                <Carousel responsive={responsive} className='z-10'>
                    {items.map((item, index) => (
                        ...
                        className="flex w-full justify-center key={index} aspect-[3/5] max-sm:aspect-[1/2] relative brightness-90 group object-contain ">
                            ...
                            />
                            <div onClick={() => setOpen(!open)} className="absolute inset-0 bg-gradient-to-b from-transparent group-hover:from-black/20 group-hover:via-black/50 group-hover:to-black/80"></div>
                            <div className=' absolute group-hover:block bottom-7 left-6 max-md:text-center max-md:left-1/2 max-md:-translate-x-1/2 translate-y-[200%] group-hover:translate-y-0 transition-all duration-500'>
                                <span className='lg:text-xl max-md:text-md font-bold uppercase block'>{item.name}</span>
                ...
                <div className={`flex absolute max-md:flex-col max-md:items-center top-0 z-20 w-full border-[2px] h-full max-md:min-h-[400px] bg-indigo-900/50 rounded-xl duration-1000 ${open ? 'block' : 'hidden'}`}
                    ...
                    <div className=" w-1/3 h-full max-md:h-2/3 max-md:w-full flex items-center justify-center">
                        <img src={Risu} alt="" className='mx-auto w-72 max-md:w-36 items-center' />
                    </div>
                    <div className="flex flex-col w-2/3 max-md:w-full h-full px-3 lg:py-5 max-md:my-2 justify-center lg:text-lg max-lg:text-md">
                        <h2 className='lg:text-4xl max-lg:text-2xl max-md:text-xl uppercase text-center font-semibold'>Ariya Risu</h2>
                        ...
                        <button className='w-40 mx-auto h-auto py-1.5 px-4 mt-8 max-md:mt-0  bg-white/50 hover:bg-white/30 rounded-xl max-md:rounded-md border-white border-2 font-semibold text-md max-md:text-sm shadow-blue-800 shadow-2xl'>
